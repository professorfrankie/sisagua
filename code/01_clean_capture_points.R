# Clean SISAGUA capture points files -----
library("dplyr")
library("readr")
library("tidyr")
library("sf")
...
# save results with years
write_rds(capture_points, "/data/brazil_health/SISAGUA/capture_points_withyears.RDS")


# summarize dataset across years
capture_points_summary <- capture_points |>
  ...
  summarise(muni_code = first(na.omit(muni_code)), cnpj_office = first(na.omit(cnpj_office)), 
            ...
            permit_drink = last(na.omit(permit_drink)), 
            lat = first(na.omit(lat)), lon = first(na.omit(lon)), 
            flow = mean(flow))


# create location dummies if muni codes match with location
munis <- readRDS("/data/brazil/spatial.rds") |> 
  select(muni_id, geometry)
country_brazil <- geobr::read_country(year = 2020)
st_crs(country_brazil) == st_crs(munis)

cp_sf <- capture_points_summary |>
  filter(!is.na(lon) & !is.na(lat)) |>
  st_as_sf(coords = c("lon", "lat"), crs = st_crs(munis), remove = FALSE)
...
write_rds(cp_final, "/data/brazil_health/SISAGUA/capture_points_cleaned.RDS")
